...
```mermaid
flowchart TD
    subgraph GOV["🏛️ GOVERNANCE & CONTROL"]
        G1["Bendahara 1 'Geset': 3 Tahun Tanpa Laporan Tertulis"]
        G2["Ketiadaan Kebijakan Backup Data & SOP Kas Rutin"]
        G3["Validasi Kemiskinan Subjektif (Tanpa DTKS)"]
    end

    subgraph PPL["👥 PEOPLE & ORGANISASI"]
        P1["Key-Person Dependency: Sekretaris Rangkap 6 Tugas (22 Tahun)"]
        P2["Perekrutan Wakil Bendahara Baru (Solusi Tambal Sulam)"]
        P3["Pemisahan Marbot Fisik vs Urusan Administrasi"]
    end

    subgraph PRC["📋 PROCESS & ALUR KERJA"]
        PR1["Double Entry Data Qurban: WA ➔ Buku Tulis ➔ Komputer"]
        PR2["Pencatatan Takjil Tertutup di Tangan Seksi Konsumsi"]
